...
test.before(async t => {
  ...
  await ctx::compile_fixture('locales--scope')
  ctx.index_path = `${ctx.public_dir}/index.html`
})

test('should render the content type locale, not the global', async t => {
  t.false(await helpers.file.contains(ctx.index_path, ctx.title[1], { async }))
  t.true(await helpers.file.contains(ctx.index_path, ctx.title[2], { async }))
  t.false(await helpers.file.contains(ctx.index_path, ctx.body[1], { async }))
  t.true(await helpers.file.contains(ctx.index_path, ctx.body[2], { async }))
})
